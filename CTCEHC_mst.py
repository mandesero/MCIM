...
class CTCEHC:
    ...
    def __preliminary_partition(self, G):
    ...
    def __generate_small_tree(self):
        ...
        for e in self.mst.edges:
            if self.labels[e[0]] != self.labels[e[1]]:
                u = self.labels[e[0]]
                v = self.labels[e[1]]
                G.add_edge(u, v, bw=self.mst.edges[e[0], e[1]]["bw"])
        return G
    ...
    #     self.__preliminary_partition(G)
    ...
    def fit_predict(self, T: nx.Graph):
        G = T.copy()
        old_to_new_names = {name: i for i, name in enumerate(G.nodes)}
        G = nx.relabel_nodes(G, old_to_new_names)
        self.__preliminary_partition(G)
        ...
        return [self.labels[old_to_new_names[name]] for name in T.nodes]
    
def CTCEHC_part(G, n):
    model = CTCEHC(n_clusters=n)
